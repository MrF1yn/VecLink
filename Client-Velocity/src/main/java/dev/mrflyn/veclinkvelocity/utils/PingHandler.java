    ...
    public ServerPing handlePing(ServerPing serverPing){
        ...
        if(this.modifyMOTD){
            if(this.displayModeMOTD==-1) {
                ...
                builder.description(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.motdList.get(randomIndex))));
            }
            else {
                try {
                    builder.description(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.motdList.get(this.displayModeMOTD))));
                }catch (Exception ex){
                    if(Main.config.getMainConfig().getBoolean("debug")){
                        ex.printStackTrace();
                    }
                    VecLinkMainVelocity.plugin.log("Error trying to parse motd.");
                    builder.description(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.motdList.get(0))));
                }
            }
        }
        if(this.modifySP){
            ...
            if (this.displayModeSP==-1) {
                ...
                builder.samplePlayers(
                        new ServerPing.SamplePlayer(
                                VecLinkMainVelocity.getLCS().serialize(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.spList.get(randomIndex)))),
                                UUID.randomUUID()));
            }
                ...
                for(String s : this.spList){
                    builder.samplePlayers(
                            new ServerPing.SamplePlayer(
                                    VecLinkMainVelocity.getLCS().serialize(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, s))),
                                    UUID.randomUUID()));
                }
            }
            else{
                try {
                    builder.samplePlayers(new ServerPing.SamplePlayer(
                            VecLinkMainVelocity.getLCS().serialize(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.spList.get(this.displayModeSP)))),
                            UUID.randomUUID()));
                } catch (Exception ex) {
                    if(Main.config.getMainConfig().getBoolean("debug")){
                        ex.printStackTrace();
                    ...
                    VecLinkMainVelocity.plugin.log("Error trying to parse motd.");
                    builder.samplePlayers(new ServerPing.SamplePlayer(
                            VecLinkMainVelocity.getLCS().serialize(VecLinkMainVelocity.getMiniMessage().deserialize(PlaceholderAPI.setPlaceholdersIfAvailable(null, this.spList.get(0)))),
                            UUID.randomUUID()));
                }
            }
        }
