...
//        .build();
    ...
    public String onRequest(Player player, @NotNull String identifier){
        ...
        identifier = FalconMainVelocity.plugin.papi.setBracketPlaceholders(player, identifier);
        ...
        if(values[1].isEmpty()){
            FalconMainVelocity.plugin.log(placeholder+ " Not allowed placeholder-syntax '%%math_<text>_%%'");
        ...
        if(isNullOrEmpty(options[0])){
            ...
            try{
                ...
                FalconMainVelocity.plugin.log(placeholder+" is not a valid number for precision! "+ options[0]);
                return null;
            }
        }
    ...
    private String evaluate(String placeholder, String expression, int precision, RoundingMode roundingMode){
        try{
            ...
            FalconMainVelocity.plugin.log(placeholder+ " is not a valid Math Expression. "+ expression);
            return null;
        }
    }
