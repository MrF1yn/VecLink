    ...
    public boolean onSubCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(args.length==1){
            ...
            if(ConnectedFalconClient.CFC.containsKey(name)){
                ...
                sender.sendMessage(ChatColor.AQUA + "BelongingGroups: " + ChatColor.WHITE + FalconMainSpigot.plugin.groups);
                sender.sendMessage(ChatColor.AQUA + "Is-Joinable: " + ChatColor.WHITE + FalconMainSpigot.plugin.isJoinable());
                ...
                sender.sendMessage(ChatColor.RED+"That Client doesn't exist or is not connected to the FalconCloudServer.");
            }
            return true;
        }
