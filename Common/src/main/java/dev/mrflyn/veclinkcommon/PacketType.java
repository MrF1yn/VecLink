...
//PC2S means the packet is only sent from a proxyVecLinkClient like Velocity or Bungee.
    ...
    S2C_REMOTE_CMD("This packet is forwarded from the FalconServer to the target VecLinkClient."),
    S2C_CLIENT_INFO("This packet is forwarded to all clients when a client connects to the veclinkServer."),
    S2C_GROUP_INFO("This packet is forwarded to a client when it connects to the veclink server. It contains the channelGroup Names."),
    S2C_PLAYER_INFO("This packet is forwarded to a client when it connects to the veclink server and also when the Server receives a C2S_PLAYER_INFO packet. " +
